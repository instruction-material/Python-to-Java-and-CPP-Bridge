...
public class JS6_Picasso extends Application {
    ...
    public void drawSquare(int red, int green, int blue, int x, int y,
                           int sideLength, Pane root) {
        Rectangle square = new Rectangle(x, y, sideLength, sideLength);
        square.setFill(Color.rgb(red, green, blue));
        root.getChildren().add(square);
    }

    public void drawCircle(int red, int green, int blue, int x, int y,
                           int diameter, Pane root) {
        Circle circle =
            new Circle(x + diameter / 2.0, y + diameter / 2.0, diameter / 2.0);
        circle.setFill(Color.rgb(red, green, blue));
        root.getChildren().add(circle);
    }

    public void drawPacman(int red, int green, int blue, int x, int y,
                           int diameter, Pane root) {
        Arc pacman = new Arc(x + diameter / 2.0, y + diameter / 2.0,
                             diameter / 2.0, diameter / 2.0, 50, 270);
